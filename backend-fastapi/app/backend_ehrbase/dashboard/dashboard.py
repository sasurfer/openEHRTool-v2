...
async def get_dashboard_data_ehrbase(request: Request,auth : str,url_base: str,url_base_status:str,url_base_management:str,app: FastAPI = Depends()):
    ...
    async with httpx.AsyncClient() as client:
        ...
        data['q']=aqltext
        headers={'Authorization':auth,'Content-Type': 'application/json'}
        response = await fetch_post_data(client=client,url=myurl,headers=headers,data=json.dumps(data))
        if 199 < response.status_code < 210:
        ...
        response=await fetch_get_data(client=client,url=myurl,headers=headers,params=params)
        if 199 < response.status_code < 210:
        ...
        response = await fetch_get_data(client=client,url=url_base_status, headers=headers)
