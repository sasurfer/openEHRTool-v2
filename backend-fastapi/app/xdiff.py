...
def canonicalize_tree(tree):
    ...
    string = etree.tostring(
        etree.XML(string), encoding="utf-8", pretty_print=True
    ).decode()
...
def split_string_into_words(string):
    prefix = re.match(r"\s*", string).group(0)
    ...
    list = [unescape_spaces_in_string(item) for item in escaped_string.split()]

    list = [prefix + item for item in list]
    return list
...
def diff_strings(string1, string2):
    ...
    context = 0
    diff = unified_diff(list1, list2, fromfile="xml1", tofile="xml2", n=context)
    return diff
