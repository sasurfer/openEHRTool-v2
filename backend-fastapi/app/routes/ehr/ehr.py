...
from app.backend_ehrbase.ehr.ehr import get_ehr_by_ehrid_ehrbase,get_ehr_by_sid_sns_ehrbase,post_ehr_by_ehrid_ehrbase,post_ehr_by_sid_sns_ehrbase
import redis
...
async def post_ehr_by_sid_sns_with(request: Request, subjectid: str, subjectnamespace: str, ehrid: UUID, redis_client: redis.StrictRedis = Depends(get_redis_client), token: str = Depends(get_token_from_header)):
    logger=get_logger(request)
    ...
    try:
        ehrid=str(ehrid)
        url_base = request.app.state.url_base
        response=await post_ehr_by_sid_sns_ehrbase(request,auth,url_base,ehrid,subjectid,subjectnamespace)
        ehrid=response['ehrid']
        insertlogline(redis_client,'Post EHR by subjectid and subjectnamespace: ehr '+ehrid+' subjectid='+subjectid+' and subjectnamespace='+subjectnamespace+' posted successfully')
        return JSONResponse(content={"ehr": response['ehr']}, status_code=200)
    except Exception as e:
        logger.error(f"An exception occurred during post_ehr_by_sid_sns_with: {e}")
        if 400 <= e.status_code < 500 :
            insertlogline(redis_client,f'Post EHR by subjectid and subjectnamespace: subjectid={subjectid} and subjectnamespace={subjectnamespace} not successful')
            return JSONResponse(content={"ehr": e.__dict__}, status_code=e.status_code)
        else:
            print(f"An exception occurred during post_ehr_by_sid_sns_with: {e}")
            raise HTTPException(status_code=500, detail="Server error during post_ehr_by_sid_sns_with")        
        
@router.post("/subjectid/{subjectid}/subjectnamespace/{subjectnamespace}")
async def post_ehr_by_sid_sns_without(request: Request, subjectid: str, subjectnamespace: str, redis_client: redis.StrictRedis = Depends(get_redis_client), token: str = Depends(get_token_from_header)):
    logger=get_logger(request)
    ...
    try:
        ...
        if e.status_code == 404:
            ...
            raise HTTPException(status_code=500, detail="Server error during post_ehr_by_sid_sns_without")      