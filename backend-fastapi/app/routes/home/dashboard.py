...
@router.get("")
async def get_dashboard_data(request: Request, token: str = Depends(get_token_from_header)):
    ...
    try:
        ...
        logger.debug('before get_dashboard_data_ehrbase')
        metrics, barData, pieData, info = await get_dashboard_data_ehrbase(request,auth, url_base, url_base_status, url_base_management)
        return {"dashboard_data": {"metrics": metrics, "barData": barData, "pieData": pieData, "info": info}}

    except Exception as e:
        print(f"An exception occurred during get_dashboard_data: {e}")
        raise HTTPException(status_code=500, detail="Server error during get_dashboard_data")